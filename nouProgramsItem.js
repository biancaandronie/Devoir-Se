...
export default class ProgramsItem extends Component{
    ...
    openItem(event, item, i){
        if(this.props.type === "replay-program"){
            return this.openReplayItem(event, item);
        }else if(this.props.type === "tv-program"){
            return this.openTVItem(event, item, i);
        }
    }
    ...
    createCarouselContent() {
        let { isStripContent, isTvContent, isReplayContent, isMosaic, program, index, elementsIdPrefix, channel, snDown, snUp, isBackgroundBlack, marginStyle, channels, subscribed } = this.props;
        ...
        if(program) {
            ...
            return (
                <div className="program-item-container" style={marginStyle}>
                    ...
                    { isTvContent && program.liveProgress && <div className="progress-bar-container"><div className="progress-bar" style={{width: program.liveProgress+'%'}}></div></div> }


                    { isReplayContent && <div className="program-details-container">
                        <label className={"overflow-text two-lines-text bold-text size-24 "+labelTextColor}>{getProgramTitle(program)}</label>
                        { isStripContent && category && <label className={"program-genre bold-text size-20 " + labelTextColor}>{category}</label>}
                    </div> }

                    { isTvContent && <div className="program-details-container flex-space-between">
                        ...
                        <div className="program-channel-logo-container center-image" style={{backgroundSize: "70px", backgroundImage: 'url("' +  getChannelCover(channels[index]) + '")'}}/>
                    </div> }
